...
agent_runtime/ agent lane: typed session requests, security kernel, auth
               isolation, event normalization, shared Codex App Server control,
...
Exactly two routes ship: `(codex, sdk)` and `(claude, sdk)`. The Codex extra
declares `websockets`; the host separately pins and supervises Codex 0.153.4.
The Codex route retains the `sdk` name for reference compatibility and owns the
documented WebSocket-over-Unix-socket App Server client; Claude remains on its
