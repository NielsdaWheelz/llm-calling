...
class CodexAppServerClient:
    ...
    def __init__(self, config: CodexAppServerConfig) -> None:
        ...
        self._messages: asyncio.Queue[CodexServerMessage | _TransportFailure] = asyncio.Queue()
    ...
    async def request(self, method: str, params: Mapping[str, object] | None) -> object:
        ...
        try:
            ...
            raise ProtocolDefect(f"Codex app-server {method} request timed out") from None
    ...
    async def _route_method(self, message: dict[str, object]) -> None:
        ...
        if "id" not in message:
            await self._messages.put(CodexNotification(method=method, params=params))
            return
