...
- Failure taxonomy: `TransientCause` leaves, `ProviderContextTooLarge`, `InvalidToolArguments`, `InvalidStructuredOutput`, `TransientExhausted` — closed unions; plus the `FailureOrigin`/`FailureCode` ledger vocabularies (`FailureOrigin` is carried by every `RuntimeDefect`; the runtime maps no failure value to a code itself).
...
| `IntentContextTooLarge` failure leaf | **deleted** — `ProviderContextTooLarge`, origin `provider_http`, is the only context-overflow signal; the local pre-network measurement it presupposed does not exist in v2. Ledger writes that branched on the intent-origin overflow collapse to the provider one. The `failure_origin`/`failure_code` mapping helpers go with it: a consumer that writes those columns owns the mapping against the closed unions |
...
| Local (pre-network) context-overflow signal | `IntentContextTooLarge` deleted from the taxonomy: nothing in the lane measures a prompt before dispatch, so the leaf was unconstructible and its `"intent"` origin a fiction. `ProviderContextTooLarge` is the single overflow signal. The `failure_origin`/`failure_code` helpers are deleted with it — test-only reachable, absent from the facade; `FailureOrigin` survives as the `RuntimeDefect` field | owner, from review round 3 |
| In-band provider error in an HTTP 200 body | Without a definite 4xx code in the body, classify **transient**. A 200 that carries an error is an ambiguous signal, and treating ambiguity as permanent burns a retry the provider may well honor; a definite 4xx code in the body classifies as that code does | owner, from review round 3 |
| `reasoning="none"` on a row declaring no `"none"` fragment | Send NO reasoning field — the provider default applies — and never raise. "none" is the facade default (`chat()`), so raising would make knobless and none-less rows uncallable through the sugar. Non-`none` levels a row does not declare still raise `InvalidRequest` (row above): a caller who asks for an explicit non-default effort must not be silently downgraded | owner, from review round 3 |
