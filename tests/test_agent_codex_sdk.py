...
from provider_runtime.agent_runtime.codex_app_server import CodexNotification, CodexServerRequest
...
def notification(method: str, payload: dict[str, object]) -> CodexNotification:
    return CodexNotification(method=method, params=payload)
...
class FakeTurn:
    ...
    def _turn_completed(self, status: str, **extra: object) -> CodexNotification:
    ...
    def _usage_notification(self, delta: dict[str, int] | None = None) -> CodexNotification:
    ...
    async def stream(self):
        ...
        late_replay = cast(
            dict[str, CodexNotification], sdk_state(self._module)["late_restored_usage"]
    ...
    *,
    ...
    class FakeAsyncCodex:
        def __init__(self, config: Any):
            ...
            self.closed = False
            self._pending_messages: list[CodexNotification] = []
        ...
        def take_pending_messages(self) -> tuple[CodexNotification, ...]:
        ...
        def _queue_restored_usage(self, thread_id: str) -> None:
            ...
            if state["defer_restored_usage"]:
                cast(dict[str, CodexNotification], state["late_restored_usage"])[thread_id] = replay
            else:
...
async def test_model_catalog_accepts_initialize_user_agent_without_server_info(
    ...
    """Read the certified initialize and model/list shapes over a real stdio child."""
...
for line in sys.stdin:
    ...
    if method == "initialize":
        assert request["params"]["capabilities"] == {"experimentalApi": False}
        assert "PROVIDER_CATALOG_AMBIENT_SECRET" not in os.environ
        result = {"userAgent": "provider_runtime/0.144.4 (macOS; arm64) unknown"}
    elif method == "account/read":
        result = {"account": {"type": "chatgpt"}}
    elif method == "model/list":
        assert request["params"] == {"includeHidden": False, "cursor": None}
        result = {"data": [{
...
""",
    ...
    sdk = fake_sdk()
    runtime_package = fake_runtime_package()
    runtime_package.__dict__["bundled_codex_path"] = lambda: executable
    monkeypatch.setitem(sys.modules, "openai_codex", sdk)
    monkeypatch.setitem(sys.modules, "codex_cli_bin", runtime_package)
    monkeypatch.setenv("PROVIDER_CATALOG_AMBIENT_SECRET", "must-not-reach-child")
    state_root = tmp_path / "codex-state"
    state_root.mkdir(mode=0o700)
    adapter = CodexSdkAdapter()
    try:
        observed = await adapter.model_catalog(
    ...
    assert observed.models[0].dispatch_model == "gpt-test"
    assert observed.models[0].source_default_reasoning == Present("medium")
...
async def test_typed_tmpdir_and_workspace_write_controls_reach_every_open_path(
    ...
    assert all(
        client.config.environment["TMPDIR"] == str(child_tmpdir.resolve()) for client in clients
    )
