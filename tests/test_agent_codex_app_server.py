...
def test_retained_incident_fixture_contains_only_sanitized_shape_and_hash_evidence() -> None:
    retained = cast(dict[str, object], _CASES["retained_incident_shape"])
    items = cast(list[dict[str, object]], retained["items"])
    synthetic = cast(list[dict[str, object]], _CASES["custom_exec_lifecycle"])

    assert retained["source_record_indices"] == [15, 16]
    assert [item["type"] for item in items] == [
        "custom_tool_call",
        "custom_tool_call_output",
    ]
    assert items[0]["call_id_json_sha256"] == items[1]["call_id_json_sha256"]
    assert items[1]["output_length"] == 2
    assert all("input" not in item and "output" not in item for item in items)
    assert [
        cast(dict[str, object], cast(dict[str, object], message["params"])["item"])["type"]
        for message in synthetic
    ] == ["custom_tool_call", "custom_tool_call", "custom_tool_call_output"]


def test_protocol_classification_fixture_covers_every_explicit_transport_family() -> None:
    classification = cast(dict[str, list[str]], _CASES["protocol_classification"])

    assert set(classification["inert_item_types"]) == codex_sdk_module._INERT_ITEM_TYPES
    assert set(classification["authority_item_types"]) == {
        *codex_sdk_module._AUTHORITY_ITEM_TYPES,
        "custom_tool_call",
        "custom_tool_call_output",
    }
    assert set(classification["inert_notification_methods"]) == (
        codex_sdk_module._INERT_NATIVE_METHODS
    )
    assert set(classification["forbidden_session_notifications"]) == (
        codex_sdk_module._FORBIDDEN_SESSION_NOTIFICATIONS
    )
    assert set(classification["denied_server_requests"]) == set(
        app_server_module._SERVER_REQUEST_DENIALS
    )
    assert set(classification["forbidden_server_requests"]) == {
        *app_server_module._FORBIDDEN_SERVER_REQUESTS,
        *app_server_module._FORBIDDEN_AUTHORITY_REQUESTS,
    }
