...
async def test_stream_success() -> None:
    ...
    async with httpx.AsyncClient() as http:
        events = [
            event
            async for event in AnthropicClient(http).generate_stream(
                request(), api_key="sk-test", timeout_s=30
            )
        ]

    done = terminal(events)
    assert done.provider_request_id == "msg_test123"
...
async def test_tool_use_streaming_emits_tool_call_chunk() -> None:
    ...
    assert len(tools) == 1
    assert tools[0] == ToolCall(id="toolu_xyz", name="get_weather", arguments={"city": "SF"})
...
async def test_stream_thinking_blocks_yield_provider_artifact_chunks() -> None:
    ...
    assert stream_text(events) == "Answer."
    assert terminal(events).terminal is True
