...
async def test_generate_retries_retryable_errors_before_success() -> None:
    ...
    assert response.attempts[0].safe_body_snippet is not None
    assert "server_error" in response.attempts[0].safe_body_snippet
    assert response.attempts[1].provider_request_id == "req-after-retry"
    assert response.attempts[1].safe_body_snippet is None
